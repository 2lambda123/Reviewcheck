...
def show_reviews(config: Dict[str, Any], suppress_notifications: bool) -> None:
    ...
    hide_all_threads_you_already_replied_to = config["hide_replied_discussions"]
    ...
    for mr in mrs:
        if not (mr.user_reacted_but_no_upvote() and not mr.is_author):
            if mr.number_of_your_open_threads == 0:
                continue
            if (
                mr.number_of_open_threads_needing_your_reply == 0
                and not show_all_discussions
            ):
                continue

        if mr.user_reacted_but_no_upvote() and mr.number_of_open_threads > 0:
            continue
        ...
        if mr.user_reacted_but_no_upvote() and not mr.is_author:
            continue
        ...
        for thread in mr.threads:
            ...
            if hide_all_threads_you_already_replied_to:
                if last_message["author"]["username"] == you:
                    continue

            you_need_to_reply = False
            if last_message["author"]["username"] != you:
                you_need_to_reply = True
            ...
            border_color = f"{main_mr_color}" if you_need_to_reply else "white"
            ...
            if you_need_to_reply or show_all_discussions:
                thread_table.add_row(
                    "",
                    "Discussion link",
