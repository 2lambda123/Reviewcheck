...
class MergeRequest:
    ...
    def __init__(self, threads: Any, reactions: Any, metadata: Any, user: str):
        ...
        self.you: str = user
        self.is_author = self.mr_author == self.you
        ...
        self.number_of_your_open_threads = 0
        self.number_of_open_threads_needing_your_reply = 0
        ...
        for thread in threads:
            ...
            if "resolved" in first_message:
                ...
                if not first_message["resolved"]:
                    ...
                    if (
                        self.you_are_participant(messages)
                        or self.you_are_referenced_in_thread(messages)
                        or self.you_are_author_of_thread(messages)
                    ):
                        self.number_of_your_open_threads += 1
                        self.threads.append(thread)
                        self.all_last_message_ids.append(last_message["id"])
                        if last_message["author"]["username"] != self.you:
                            self.number_of_open_threads_needing_your_reply += 1

        self.reaction_and_name: DefaultDict[str, List[str]] = defaultdict(list)
        self.reaction_and_user: DefaultDict[str, List[str]] = defaultdict(list)
        for reaction in reactions:
            self.reaction_and_name[reaction["name"]].append(reaction["user"]["name"])
            self.reaction_and_user[reaction["name"]].append(
    ...
    def user_reacted_but_no_upvote(self) -> bool:
        ...
        """
        has_reacted = any(self.you in r for r in self.reaction_and_user.values())
        has_upvoted = self.you in self.reaction_and_user.get("thumbsup", [])
        if has_reacted and not has_upvoted:
            return True
        return False

    def you_are_referenced_in_thread(self, messages: List[Dict[str, Any]]) -> bool:
        ...
        for message in messages:
            if ("@" + self.you) in message["body"]:
                return True
        return False

    def you_are_author_of_thread(self, messages: List[Dict[str, Any]]) -> bool:
        ...
        :return: False if the user doesn't have anything to respond to,
            otherwise True.
        """
        if self.mr_author == self.you:
            if messages[-1]["author"]["username"] != self.you:
                return True
        return False

    def you_are_participant(self, messages: List[Dict[str, Any]]) -> bool:
        ...
        """
        for message in messages:
            if message["author"]["username"] == self.you:
                return True
        return False
